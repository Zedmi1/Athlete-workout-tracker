  ...
  try {
    ...
    if (sports && sports.length > 0) {
      for (const sportName of sports) {
        const sportResult = await db.query('SELECT id FROM sports WHERE name = $1', [sportName]);
...
app.get('/api/exercises', async (req, res) => {
  const exercisesBySport = {
    'Football': ['Sprint Drills', 'Ball Control Training', 'Tactical Session', 'Shooting Practice', 'Passing Drills'],
    'Basketball': ['Free Throw Practice', 'Defensive Drills', 'Scrimmage Game', 'Dribbling Drills', 'Layup Practice'],
    'Athletics': ['Track Intervals', 'Long Distance Run', 'Hill Sprints', 'Speed Work', 'Endurance Training'],
    'Swimming': ['Freestyle Intervals', 'Endurance Session', 'Mixed Stroke Practice', 'Butterfly Drills', 'Backstroke Training'],
    'Cycling': ['Hill Climbing', 'Interval Training', 'Long Ride', 'Sprint Training', 'Time Trial'],
    'Tennis': ['Serve Practice', 'Match Play', 'Footwork Drills', 'Backhand Training', 'Volley Practice'],
    'Volleyball': ['Spike Training', 'Team Practice', 'Blocking Technique', 'Serving Drills', 'Passing Practice'],
    'Golf': ['Driving Range', 'Putting Practice', '18 Holes', 'Chipping Practice', 'Iron Training']
  };
...
app.post('/api/workouts', async (req, res) => {
  ...
  try {
    const sportResult = await db.query('SELECT id FROM sports WHERE name = $1', [sport]);
    ...
    const result = await db.query(
      ...
      [userId, exercise, sportId, sets || null, reps || null, distance || null, duration || null, notes || null]
    );
    ...
    res.status(201).json({
      ...
      notes: workout.notes,
      date: new Date(workout.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
...
app.post('/api/workouts/:id/like', async (req, res) => {
  ...
  try {
    ...
    if (existingLike.rows.length > 0) {
      ...
      await db.query(
        'UPDATE workouts SET likes = likes - 1 WHERE id = $1',
        [workoutId]
      );
...
async function initializeDatabase() {
  try {
    const checkTable = await db.query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'sports')");
...
async function createSampleWorkouts() {
  try {
    ...
    for (const user of sampleUsers) {
      ...
      const sportResult = await db.query('SELECT id FROM sports WHERE name = $1', [user.sport]);
      ...
      for (const workout of workouts) {
        await db.query(
          ...
          [userId, workout.exercise, sportId, workout.sets, workout.reps, workout.distance, workout.duration, workout.notes, new Date(Date.now() - workout.daysAgo * 24 * 60 * 60 * 1000)]
        );
      }
    }
...
function getSampleWorkoutsForSport(sport) {
  const workoutData = {
    'Football': [
      { exercise: 'Sprint Drills', sets: '5', reps: '10', distance: '100m', duration: '45 min', notes: 'Focused on acceleration', daysAgo: 1 },
      { exercise: 'Ball Control Training', sets: null, reps: null, distance: null, duration: '60 min', notes: 'Dribbling and passing drills', daysAgo: 3 },
      { exercise: 'Tactical Session', sets: null, reps: null, distance: '5km', duration: '90 min', notes: 'Team formation practice', daysAgo: 5 }
    ],
    'Basketball': [
      { exercise: 'Free Throw Practice', sets: '10', reps: '20', distance: null, duration: '30 min', notes: '85% accuracy today', daysAgo: 1 },
      { exercise: 'Defensive Drills', sets: null, reps: null, distance: null, duration: '45 min', notes: 'One-on-one defense focus', daysAgo: 2 },
      { exercise: 'Scrimmage Game', sets: null, reps: null, distance: null, duration: '60 min', notes: 'Full court 5v5', daysAgo: 4 }
    ],
    'Athletics': [
      { exercise: 'Track Intervals', sets: '8', reps: '400m', distance: '3.2km', duration: '40 min', notes: 'Hit all target times', daysAgo: 1 },
      { exercise: 'Long Distance Run', sets: null, reps: null, distance: '10km', duration: '50 min', notes: 'Easy pace recovery run', daysAgo: 3 },
      { exercise: 'Hill Sprints', sets: '6', reps: '10', distance: '200m', duration: '35 min', notes: 'Tough but good session', daysAgo: 6 }
    ],
    'Swimming': [
      { exercise: 'Freestyle Intervals', sets: '10', reps: '100m', distance: '1000m', duration: '45 min', notes: 'Great technique today', daysAgo: 1 },
      { exercise: 'Endurance Session', sets: null, reps: null, distance: '2000m', duration: '55 min', notes: 'Steady pace throughout', daysAgo: 2 },
      { exercise: 'Mixed Stroke Practice', sets: '8', reps: '50m', distance: '400m', duration: '40 min', notes: 'Butterfly still needs work', daysAgo: 5 }
    ],
    'Cycling': [
      { exercise: 'Hill Climbing', sets: null, reps: null, distance: '25km', duration: '75 min', notes: 'Conquered the big hill!', daysAgo: 1 },
      { exercise: 'Interval Training', sets: '12', reps: '1min', distance: '30km', duration: '60 min', notes: 'High intensity intervals', daysAgo: 3 },
      { exercise: 'Long Ride', sets: null, reps: null, distance: '80km', duration: '180 min', notes: 'Beautiful weather', daysAgo: 7 }
    ],
    'Tennis': [
      { exercise: 'Serve Practice', sets: '10', reps: '20', distance: null, duration: '45 min', notes: 'Working on second serve', daysAgo: 1 },
      { exercise: 'Match Play', sets: '3', reps: null, distance: null, duration: '120 min', notes: 'Won 2-1 in sets', daysAgo: 2 },
      { exercise: 'Footwork Drills', sets: '8', reps: '15', distance: null, duration: '40 min', notes: 'Agility improving', daysAgo: 4 }
    ],
    'Volleyball': [
      { exercise: 'Spike Training', sets: '15', reps: '10', distance: null, duration: '50 min', notes: 'Power and accuracy focus', daysAgo: 1 },
      { exercise: 'Team Practice', sets: null, reps: null, distance: null, duration: '90 min', notes: 'Rotation drills and scrimmage', daysAgo: 3 },
      { exercise: 'Blocking Technique', sets: '10', reps: '12', distance: null, duration: '45 min', notes: 'Timing is getting better', daysAgo: 5 }
    ],
    'Golf': [
      { exercise: 'Driving Range', sets: null, reps: '100', distance: null, duration: '60 min', notes: 'Driver felt great today', daysAgo: 1 },
      { exercise: 'Putting Practice', sets: '20', reps: '10', distance: null, duration: '45 min', notes: 'Working on distance control', daysAgo: 2 },
      { exercise: '18 Holes', sets: null, reps: null, distance: '6km', duration: '240 min', notes: 'Shot 84, happy with that', daysAgo: 4 }
    ]
  };
